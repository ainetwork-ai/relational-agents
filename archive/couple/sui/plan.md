...
- [x] **M3 — Full demo on testnet (chain-side).** The judge walkthrough is written in [`sui/DEMO.md`](DEMO.md) and every step in it is runnable: `lifecycle.mjs` replays birth → memory → the non-member abort → a second relationship → dissolve, and `walrus-seal.mjs` replays seal → store → member read → outsider refusal → wrong-relationship refusal → ciphertext-on-disk. *Still open (deliberately, see PROOF.md scope):* zkLogin/Enoki onboarding and the in-app sealed-read UI — members are ed25519 keypairs today and the live pipeline still writes through `okf-store`.
