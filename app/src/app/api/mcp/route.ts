...
type Identity = {
  ...
  /** an agent token's owner — the person the agent reads for (page-to-prompt) */
  ownerId?: string;
};
...
const handler = createMcpHandler(
 ...
 // ── app-fetch ───────────────────────────────────────────────────────
    ...
    server.tool(
      "page-to-prompt",
      "Turn a page, database or block into an AI-ready prompt (notion2prompt): its blocks, child pages down to `depth`, child databases as tables, properties and metadata, in the claude-xml, default or markdown template. `page` is an id (Postgres uuid or OKF id), a /p/<id> link or a title. Read with your access (an agent token reads as its owner): what you cannot see is left out. format='json' adds the files, counts and what was left out; stage='fetch' returns the content tree, and passing that back as `content` renders it again without reading anything.",
      {
        ...
        include_properties: z.union([z.boolean(), z.literal("auto")]).optional().describe("page Properties section (default auto)"),
      ...
      async (a) => {
        const me = who();
        const viewer = me.label === "agent" ? (me.ownerId ?? "") : requireUser();
        if (!viewer) return err("page-to-prompt needs a signed-in person or an agent token with an owner.");
        const render = {
        ...
        const readers = { viewerIds: [viewer], baseUrl: publicOrigin(), workspaceIds: await workspacesOf(viewer) };
