...
/** GET → { status, room, me } — the relation's treasury as the viewer sees it, for the Treasury page. */
export async function GET(_req: NextRequest, ctx: { params: Promise<{ roomId: string }> }) {
  ...
  const [status, people] = await Promise.all([treasuryStatus(roomId, auth.user.id), roomPeople(roomId)]);
  ...
  const body: TreasuryRoomResponse = {
    status,
    room: { id: roomId, name: access.room.name || others.join(", ") || "Relation", members: people },
