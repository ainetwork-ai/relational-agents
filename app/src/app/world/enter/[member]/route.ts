...
/**
 * GET /world/enter/<member>[?to=treasury|treasurer] — into the try-it room as
 * one of its members: the account's World ID binding is dropped (visitors share
 * these accounts), then demo login signs in and lands in the room, on its
 * treasury page, or on the treasurer tab. Locations are relative for the reason
 * demo-login gives: inside the container req.nextUrl.origin is the bind
 * address, not the public host.
 */
export async function GET(req: NextRequest, ctx: { params: Promise<{ member: string }> }) {
  ...
  const tab = req.nextUrl.searchParams.get("to");
  const to =
    tab === "treasury" ? `/treasury/${room.roomId}` : tab === "treasurer" ? `/treasury/${room.roomId}/treasurer` : `/dm/${room.roomId}`;
