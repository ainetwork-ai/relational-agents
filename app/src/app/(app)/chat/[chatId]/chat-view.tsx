...
function formatRelativeTime(iso: string, t: T): string {
  ...
  if (diffHour < 24) return t("{n} hours ago", { n: diffHour });
...
export function ChatView({ chatId }: { chatId: string }) {
 ...
 // sync the sidebar list: the first message derives the title, so refresh header/store
            ...
            setError(evt.data?.message ?? t("Something went wrong"));
            setErrorKind(
      ...
      if ((err as Error)?.name === "AbortError") {
        ...
        setError(err instanceof Error && err.message ? err.message : t("Something went wrong while sending the message"));
        setErrorKind(null);
      }
    } finally {
      setStreaming(false);
 ...
 // ESC: blur the input
  ...
  async function createPageFromContent(content: string) {
    try {
      // an untitled page stores "" and is displayed as Untitled in the current language (Notion)
    ...
    <div className="relative flex h-full min-h-0 flex-col bg-white dark:bg-[#191919]">
      <header className="flex shrink-0 items-center justify-between border-b border-neutral-200 px-4 py-3 dark:border-neutral-800">
        <div className="flex min-w-0 items-center gap-2">
          ...
          >
            {chat?.icon ? `${chat.icon} ` : ""}
            {chat?.title ?? (loading ? t("Loading…") : t("Untitled chat"))}
          </h1>
          {chat?.agentName && (
            <span
      ...
      <div className="shrink-0 border-t border-neutral-200 p-3 dark:border-neutral-800">
        ...
        {isEditing && (
          ...
          >
            <span>{t("Edit the message and send it again")}</span>
            <button
              type="button"
        ...
        <div className="relative mb-2 inline-block">
          ...
          >
            <span data-testid="context-scope-badge">
              {contextScope === "workspace" ? t("Entire workspace") : t("This page only")}
            </span>
          </button>
          {scopeMenuOpen && (
            <div
        ...
        <div className="relative flex items-end gap-2">
          ...
          <textarea
            ...
            aria-label={t("Message workspace AI")}
            value={input}
            ...
            placeholder={t("Ask workspace AI")}
            rows={1}
            disabled={composerDisabled}
        ...
        <div className="mt-1 flex items-center justify-end gap-2 px-0.5 text-[10px] text-neutral-400 dark:text-neutral-500">
          {input.length >= CHAR_WARN_THRESHOLD && (
            ...
            >
              {t("Approaching the character limit")}
            </span>
          )}
