...
// display only — the role/permission strings themselves are persisted as-is
const ROLE_LABEL: Record<string, string> = { guest: "Guest", member: "Member", admin: "Admin", owner: "Owner" };
const PERMISSION_LABEL: Record<string, string> = { view: "View", read: "Read", comment: "Comment", edit: "Edit", full: "Full access" };
 ...
 // an admin+ can manage everyone except the owner and themselves
        ...
        {(guestGrants.length > 0 || guestInvites.length > 0) && (
          ...
          >
            <div className="mb-1.5 text-xs font-medium text-neutral-500 dark:text-neutral-400">
              {t("Guest access — per-page permissions")}
            </div>
            {guestGrants.map((g) => (
              <div
              ...
              >
                <span className="min-w-0 flex-1 truncate">
                  {g.user.displayName} · <span className="text-neutral-400">{g.title || t("Untitled")}</span>
                </span>
                <span className="shrink-0 capitalize text-neutral-400">{t(PERMISSION_LABEL[g.permission] ?? g.permission)}</span>
                <button
            ...
            {guestInvites.map((g) => (
              ...
              >
                <span className="min-w-0 flex-1 truncate">
                  {g.email} <span className="text-neutral-400">({t("Pending")})</span> ·{" "}
                  <span className="text-neutral-400">{g.title || t("Untitled")}</span>
                </span>
                <span className="shrink-0 capitalize text-neutral-400">{t(PERMISSION_LABEL[g.permission] ?? g.permission)}</span>
                <button
        ...
        <div className="border-t border-neutral-100 px-4 py-3 dark:border-neutral-700">
          <div className="mb-2 flex items-center gap-2 text-xs font-medium text-neutral-500 dark:text-neutral-400">
            <UserPlus size={13} /> {t("Invite members")}
          </div>
          {inviteUrl ? (
