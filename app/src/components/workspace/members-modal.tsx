...
const ROLE_LABEL: Record<string, string> = { guest: "게스트", member: "멤버", admin: "관리자", owner: "소유자" };
const PERMISSION_LABEL: Record<string, string> = { view: "보기", read: "읽기", comment: "댓글", edit: "편집", full: "전체 권한" };
        ...
        {(guestGrants.length > 0 || guestInvites.length > 0) && (
          ...
          >
            ...
            {guestGrants.map((g) => (
              ...
              >
                <span className="min-w-0 flex-1 truncate">
                  {g.user.displayName} · <span className="text-neutral-400">{g.title || t("제목 없음")}</span>
                </span>
            ...
            {guestInvites.map((g) => (
              ...
              >
                <span className="min-w-0 flex-1 truncate">
                  {g.email} <span className="text-neutral-400">({t("대기 중")})</span> ·{" "}
                  <span className="text-neutral-400">{g.title || t("제목 없음")}</span>
                </span>
                <span className="shrink-0 capitalize text-neutral-400">{t(PERMISSION_LABEL[g.permission] ?? g.permission)}</span>
                <button
