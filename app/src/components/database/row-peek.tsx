...
function CustomizeLayout() {
 ...
 // the band's own order (config.pinnedOrder), lowest first — see splitPinned
 ...
 // dragging inside 레이아웃 사용자 지정; the order it produces is what matters,
 // so this menu moves one step at a time.
      ...
      {open && (
        ...
        >
          ...
          {[...nonTitle.map((p, i) => ({ p, i })).filter(({ p, i }) => isOn(p, i))
            .sort((a, b) => pinned.findIndex((q) => q.id === a.p.id) - pinned.findIndex((q) => q.id === b.p.id)),
            ...nonTitle.map((p, i) => ({ p, i })).filter(({ p, i }) => !isOn(p, i))]
            .map(({ p, i }) => {
            const on = isOn(p, i);
