...
/** The registry the signatures anchor to. It computes its EIP-712 domain from
 * its own address, so this must resolve identically on client and server. */
export function relationRegistryAddress(): `0x${string}` {
  return (process.env.NEXT_PUBLIC_RELATION_REGISTRY_ADDRESS ??
