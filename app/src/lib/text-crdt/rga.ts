/**
 * RGA over runs (docs/text-crdt-design.md §1.1, §6). Items are text-only runs
 * of consecutive characters by one client; formatting is a separate mark layer
 * (marks.ts). A run's first character carries the external origin, every later
 * character's origin is the character before it. Integration splits a run when
 * an insert lands in its middle and re-merges afterwards, so the list stays
 * small while every character keeps a resolvable id.
 */
...
export function integrate(items: TextItem[], item: TextItem): boolean {
  ...
  while (pos < items.length) {
    ...
    if (oOriginIdx > myOriginIdx) { pos++; continue; }
    if (oOriginIdx === myOriginIdx) {
      if (precedes(o.id, item.id)) { pos++; continue; }
      break;
    }
    break;
  }
 ...
 * same client, next seq, origin = left's last char, same liveness, no br. */
...
/** Coalesce every mergeable neighbour pair (after tombstoning). */
