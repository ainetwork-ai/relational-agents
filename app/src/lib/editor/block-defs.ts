...
import { SLASH_KEYWORDS_KO } from "@/i18n/content/editor";
...
/** Order matters: first match wins for slash-menu filtering. Labels are
 *  English t() keys; the Korean search words are appended to `keywords` below
 *  so Korean input keeps matching. */
const SLASH_ITEMS_BASE: SlashItem[] = [
  ...
  { type: "file", id: "aindrive", label: "From aindrive", keywords: "aindrive drive import link file", hint: "Link a file from aindrive", category: "aindrive", glyph: "☁", preset: { [AINDRIVE_PICK]: true } },
  ...
  { type: "toggle", label: "Toggle list", keywords: "toggle collapse fold", hint: "Hide content inside" , category: "basic", md: ">", glyph: "▸" },
  ...
  { type: "equation", label: "Equation", keywords: "equation math tex latex katex", hint: "Display a TeX equation" , category: "basic", md: "$$", glyph: "Σ" },
  { type: "button", label: "Button", keywords: "button click action automation trigger", hint: "Run actions on click" , category: "advanced", glyph: "▶" },
...
export const SLASH_ITEMS: SlashItem[] = SLASH_ITEMS_BASE.map((i) => {
  const ko = SLASH_KEYWORDS_KO[i.id ?? i.type];
  return ko ? { ...i, keywords: `${i.keywords} ${ko}` } : i;
});
...
export const MARKDOWN_SHORTCUTS: { prefix: string; type: BlockType }[] = [
  ...
  { prefix: "[]", type: "todo" },
  // Original (measured 2026-08-26): ">" is a toggle, '"' is a quote
  { prefix: ">", type: "toggle" },
  { prefix: '"', type: "quote" },
  { prefix: "```", type: "code" },
];
