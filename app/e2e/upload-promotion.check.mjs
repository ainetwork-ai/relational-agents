...
const parsed = storage.parseStorageUrl(a.url);
if (!parsed) d.push(`승격 결과가 s3:// 가 아닙니다: ${a.url}`);
...
if (b.url !== a.url) d.push(`같은 바이트인데 키가 다릅니다:\n      ${a.url}\n      ${b.url}`);
...
else if (again.url !== a.url) d.push("재실행이 다른 키를 만들었습니다 — 멱등이 아닙니다");
...
made.push(c.url);
if (c.url === a.url) d.push("다른 바이트가 같은 키를 받았습니다");
