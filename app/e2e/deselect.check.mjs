...
{ await clear(); const s = await marquee();
  ...
  const g = await page.evaluate((ids) => ids.map((id) => { const row = document.querySelector(`[data-testid="block-${id}"]`); if (!row) return null; const h = row.querySelector(":scope > div > [data-selected]"); if (!h) return null; const hr = h.getBoundingClientRect(); const rr = row.getBoundingClientRect(); const padL = row.firstElementChild ? parseFloat(getComputedStyle(row.firstElementChild).paddingLeft) : 0; const s = getComputedStyle(h); return { type: row.getAttribute("data-block-type"), padL, inset: { top: +(hr.top - rr.top).toFixed(1), left: +(hr.left - rr.left).toFixed(1), right: +(rr.right - hr.right).toFixed(1), bottom: +(rr.bottom - hr.bottom).toFixed(1) }, bg: s.backgroundColor, radius: s.borderRadius, shadow: s.boxShadow, pe: s.pointerEvents, y: hr.top, b: hr.bottom }; }), trio.map((t) => t.id));
  ...
  check("G. halo inset: 2px inside the block box (its own indent + 2 on the left), 2px top/bottom on text, 1px on list items between list items", g.every((h, i) => h && h.inset.left === h.padL + 2 && h.inset.right === 2 && h.inset.top === want(i).top && h.inset.bottom === want(i).bottom), JSON.stringify(g.map((h) => h && [h.type, h.inset])));
