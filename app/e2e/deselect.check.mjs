...
const textSel = async () => { await reScroll(); const r1 = await rect(trio[0].id), r2 = await rect(trio[1].id); await drag(r1.x + 40, r1.y + r1.h / 2, r2.x + 60, r2.y + r2.h / 2); return state(); };
{ await clear(); const b = await textSel(); const ok0 = b.anchorBlock !== b.focusBlock && !b.collapsed; const { r2 } = await rx(); await click(r2.bx - 110, r2.by + r2.bh / 2); const s = await state(); check("T1. text selection across blocks + margin click → selection gone, caret on that line", ok0 && s.collapsed === true && s.halos.length === 0, JSON.stringify({ setup: ok0, collapsed: s.collapsed, caret: s.caretBlock })); }
