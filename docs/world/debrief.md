...
- **IDKit — "is this member one unique human?"**, once, when they claim a seat.
  The seat fixes the roster before any request exists: who is notified, whether
  enough people could ever reach a bar, and "one human, one seat" on the
...
Either alone falls short: step-ups alone would let any account holder vote; a
seat alone would let a hijacked session approve.
...
the IdP, same human from a second account, unseated member, stale proof,
...
presents the same nullifier in every relation — and "one human, one seat" per
...
app-wide, the stored nullifier links one human's seats across relations on our
server. A per-relation action would remove that link at the cost of registering
an action per group. Seats accept exactly one protocol (World ID 3.0 from the
`orbLegacy` preset): a 3.0 and a 4.0 proof of the same human carry different
nullifiers, so accepting both would seat one human twice.

- **Time to first success:** _TBD_ (needs the Developer Portal staging app; so
  far only the dev simulator has seated anyone)
- **Friction:**
...
- **Missing capability / docs:** during the 3.0 → 4.0 migration there is no way
  ...
  nullifier spaces yourself; we pinned seats to one protocol instead. The rest
  _TBD_ after the staging run.
