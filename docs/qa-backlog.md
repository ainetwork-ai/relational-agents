...
- **상태**: 원인 확정. 미수정.
- **고칠 방향**: 노션 동작 = 다른 워크스페이스 페이지를 열면 **활성 워크스페이스를 그 페이지 쪽으로 자동 전환**. 대안은 안내 후 전환 또는 차단. 수정 지점은 `p/[pageId]/page.tsx` 한 곳(멤버십 확인에 더해 `page.workspaceId !== activeWorkspaceId`면 세션 활성값을 페이지 워크스페이스로 갱신하고 사이드바를 그에 맞춤).
