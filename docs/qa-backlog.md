...
## QA-3 · 워크스페이스 전환 시 페이지가 섞임 (P1) — **원인 확정, 수정에 제품 결정 필요**

- **제보**: 지영 9/8 00:04 "컴컴 워크스페이스 ↔ 개인 워크스페이스를 오갈 때 서로 페이지가 섞임. 선택된 워크스페이스는 개인용인데 컴컴 스페이스의 내용이 뜬다든지."
- **출처 메시지**: `bdb0733d`
- **먼저 배제한 후보(코드 확인)**: (1) 클라이언트 캐시 잔존 아님 — `usePagesStore.load()`는 `set({ pages: map, ...derive(map) })`로 맵을 **통째로 교체**한다(`stores/pages.ts:83-89`), 이전 워크스페이스 항목이 남지 않는다. (2) 전환 순서 정상 — 스위처가 `/api/workspaces/switch`(세션 `activeWorkspaceId` 갱신)를 await한 뒤 `refreshWorkspace()`에서 `load()`·`router.push`·`router.refresh()`를 부른다(`sidebar/workspace-switcher.tsx:96-121,89-94`). (3) Postgres 페이지는 워크스페이스로 엄격히 걸린다 — `/api/pages`가 `eq(pages.workspaceId, workspaceId)`로만 조회(`api/pages/route.ts:26-28`).
- **원인(확정)**: **OKF 파일 기반 페이지가 워크스페이스에 묶이지 않는다.** `/api/pages`는 Postgres 페이지에 더해 OKF(폴더 트리 = 콘텐츠 백엔드) 페이지를 병합하는데(`api/pages/route.ts:101-168`), 워크스페이스 필터 `inThisWorkspace()`가 **관계 문서(relationship doc)만** 실제로 거른다. 그 외 일반 OKF 콘텐츠(컴컴 Projects 등 문서 파일)는 파일명이 "관계 문서 — <상대>" 패턴이 아니라 `if (!m) return true`로 **모든 워크스페이스에서 통과**한다(`route.ts:140-146`). 게다가 `okfSyntheticPage({ workspaceId, … })`가 **지금 보고 있는 워크스페이스 id**를 그 페이지에 그대로 박아준다(`route.ts:150-163`). 그래서 개인 워크스페이스로 전환해도 컴컴 OKF 콘텐츠가 사이드바에 그대로 뜨고, 소속 워크스페이스도 개인 것으로 표시된다 — 정확히 제보한 "개인용인데 컴컴 내용이 뜬다".
- **로그/DB 확인법**: 개인 워크스페이스에서 `/api/pages` 응답에 OKF id(base64url 경로)를 가진 항목이 컴컴 문서로 남아 있는지. 그 항목들의 `workspaceId`가 개인 워크스페이스로 찍히는지(합성값이라 항상 현재 값).
- **상태**: 원인 확정. **수정은 결정 필요** — OKF는 콘텐츠 백엔드라 "모든 워크스페이스 공유"가 의도였을 수 있다. 임의로 숨기면 보이던 문서가 사라져 역효과.
- **고칠 방향(결정 대상)**: 세 갈래 중 택1 — (A) OKF 경로마다 홈 워크스페이스를 부여하고(관계 문서처럼) 그 워크스페이스에서만 노출. 매핑 규칙(폴더 → 워크스페이스)을 정해야 함. (B) OKF 콘텐츠는 "컴컴(메인) 워크스페이스"에만 노출하고 개인 워크스페이스에선 숨김. (C) 현 동작이 의도면 UI에서 "공유 콘텐츠"임을 표시해 섞임처럼 안 보이게. 코드 지점은 `api/pages/route.ts`의 `inThisWorkspace` 확장 한 곳.
