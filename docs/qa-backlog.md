...
- **상태**: 원인 확정. 미수정.
- **노션 측정(2026-09-09, CDP 읽기 전용, golden.check 통과)**: 노션은 **워크스페이스를 페이지 URL에 넣는다** — `app.notion.com/p/<workspace-slug>/<pageid>`(관측값 `/p/comcom/3c8d8655…`). 즉 페이지의 워크스페이스는 URL 세그먼트이고, 상단 브레드크럼의 **맨 앞 크럼이 그 워크스페이스**(아이콘+이름 "현정 테스트") → 팀스페이스("팀스페이스 홈") → 페이지 순으로 렌더된다. 워크스페이스 표시가 **페이지(URL)에서 파생**되므로, 노션에는 "헤더는 A인데 본문은 B 페이지" 같은 불일치가 **구조적으로 생길 수 없다**. 우리 앱은 `/p/<id>`에 워크스페이스가 없고 활성 워크스페이스가 별도 세션값이라 어긋난다.
- **고칠 방향(측정 기반)**: 노션에 맞추면 두 갈래. (A, 노션과 동형) **페이지 라우트를 워크스페이스로 스코프** — 페이지의 워크스페이스를 URL/헤더의 근거로 삼아 사이드바·활성 상태를 그 페이지에서 파생(가장 정합, 라우팅 변경 큼). (B, 최소 변경) 다른 워크스페이스 페이지를 열면 **활성 워크스페이스를 페이지 쪽으로 전환**. 단 Next 서버 컴포넌트 렌더 중엔 세션 쿠키를 못 바꾸므로, `p/[pageId]/page.tsx`에서 `page.workspaceId !== activeWorkspaceId`를 감지해 라우트 핸들러/클라이언트에서 전환을 트리거해야 한다(한 줄 아님). 우선 B로 증상 제거 후 A로 수렴 권장.
...
| QA-3 | 워크스페이스 전환 시 페이지 섞임 | P1 | 원인 확정·미수정 | 확인 |
