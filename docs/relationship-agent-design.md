...
> 새 세션은 이 문서를 읽고 §9(열린 질문)부터 이어가면 된다.
...
"로컬 모델은 실시간에 못 쓴다"는 판단은 틀렸다. 실시간 판단이 가능한 이유가 이것이다.
...
서버는 문구를 추론하지 않는다(`messages/route.ts`). §6 원칙 그대로 —
