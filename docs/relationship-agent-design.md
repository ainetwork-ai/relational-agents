...
`docs/demo.md` 1:00–2:12 장면이 사진을 **`@agent midnight natas 🥧`** 로 올리라고 한다.
그런데 `@agent` 멘션은 이제 **quiet(비밀) 대화**다 — 그 사진은 상대에게 보이지 않고
**공유 기록에도 남지 않는다.** 대본대로 찍으면 장면이 성립하지 않는다.

멘션 없이 그냥 올리면 정상 동작한다(검증됨). 대본에서 `@agent`를 빼야 한다.

### 넘길 것

- **비디오콜 담당자 메시지** — 초안 작성됨, 아직 전달 안 함. 요지 3가지:
  ① `CONTRACT.md` 12번 줄의 `"🎙 " 접두` 문구 삭제(발화가 채팅으로 안 감)
  ② Web Speech vs Azure 결정 — 전사 품질이 곧 귀띔 품질
  ③ 헤드폰은 음질이 아니라 **정확성 요건**
