...
- [x] **7. 백업 · 워치독 (2026-08-28)**
      ...
      - 워치독이 `State.Paused` 를 먼저 본다. 프로덕션 백업 중 `Paused=true,
...
- **tus 스테이징 GC 가 없다.** `.uploads-tus` 에 실패·중단된 업로드 조각이 쌓인다. 만료
  24시간은 설정해 뒀는데 청소하는 사람이 없다 — ainteams 는 `/api/cron/cleanup` 이
  `cleanUpExpiredUploads()` 를 시간마다 부른다. **내가 tus 를 들여오며 만든 빈틈이다.**
