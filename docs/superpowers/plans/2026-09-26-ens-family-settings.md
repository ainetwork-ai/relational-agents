...
| Existing demo family | onchain | `kim.ainmem.eth` (issued by the CLI under `ainmem.eth`) stays as it is; the demo workspace reaches it through its own `workspace_ens` row (data, not config) |
| Send skill | `app/src/lib/ens-chain.ts` | One family per deployment via `ENS_FAMILY_ROOT` (before this plan; removed, see F10); `createFamilyChain({ root })` works for any root name, `lee.eth` included |
...
| F10 | A workspace has a family tree only when it has its own ENS entry (a `workspace_ens` row); the tree is found from that name and built from the chain. No row → no tree, only the create form, and the send skill says this workspace has no family names yet. There is no deployment-wide fallback (`ENS_FAMILY_ROOT` is gone) | A brand-new workspace must never show another family's tree; the kim demo keeps working because grandma's demo workspace has a `workspace_ens` row for `kim.ainmem.eth` |
...
- [ ] **Step 3: `ens-workspace.ts`** (server-only): `getWorkspaceFamily(workspaceId): Promise<{ rootName: string; fromBlock: bigint } | null>`; `setWorkspaceFamily(...)` (unique violation → `"exists"`); `familyChainFor(workspaceId): Promise<FamilyChain | null>` (the mapping, else null — F10; one cached `FamilyChain` per root; `forgetFamilyChain(root)` drops the cache after writes); reservations `reserveLabel(label, workspaceId): "ok" | "reserved"`, `reservedByOther(label, workspaceId): boolean`, `releaseLabel(label)` — a `globalThis` map, entries expire after 30 minutes (F14).
- [ ] **Step 4:** Add `workspaceId` to `SendIntent` in `ens/src/send-token.ts` (update its checks, re-sync) and replace `familyChain()` in `send-by-name.ts`, `/send/page.tsx` and the confirm route with `familyChainFor(workspaceId)`. `familyChain()` and the `ENS_FAMILY_ROOT` / `ENS_FAMILY_FROM_BLOCK` reads are deleted (fix-no-default-family).
...
- [ ] **Step 4:** app `tsc`, eslint; `/browse` screenshots of states 1, 2 (with `taken`, `free` and `reserved` examples) and 5. States 3–4 need MetaMask: Task 8.
  - *Done 2026-09-26, nothing broadcast.* States 1, 2 (taken `ainmem`, invalid `li`, a free label), 4 (kim, then shown through the env fallback — removed by fix-no-default-family) and 5 screenshotted on dev 3110; with a recording EIP-1193 provider, Create holds the label and hands `deployProxy` on the VerifiableFactory to the wallet, whose rejection shows "Cancelled — press Continue". Deviations: the run orchestration (`runCreateFamily`, `runRenew`, saved run) lives in `ens-issue.ts`; `DELETE …/ens/reserve` releases the hold on Cancel / taken; (superseded by fix-no-default-family: the env fallback and its read-only "Demo family" block are gone; a workspace without its own row shows the create form only); the tree is a plain list until 7b; after the grace period the tab shows a "free again" banner but re-registering a mapped name is not wired yet (POST …/ens refuses a workspace that already has a mapping).
