...
import { SEPOLIA_EXPLORER } from "@/lib/ens-family/config";
import { formatUsdc } from "@/lib/ens-family/send-request";
import { markSent, verifySendIntent, wasSent } from "@/lib/ens-family/send-token";
...
export async function POST(req: NextRequest) {
  ...
  const url = `${SEPOLIA_EXPLORER}/tx/${body.txHash}`;
  ...
  if (intent.roomId) {
    const [bot] = await db
      ...
      .where(and(eq(chatRoomBots.roomId, intent.roomId), inArray(users.isAgent, [true])))
      .limit(1);
    if (bot) {
      await db.insert(chatMessages).values({ roomId: intent.roomId, authorId: bot.agent, text: t("💸 Sent {amount} USDC to {name}. {url}", { amount, name: intent.name, url }) });
  ...
  if (recipient && recipient.id !== intent.userId)
    await db.insert(notifications).values({
      ...
      body: t("{amount} USDC · {url}", { amount, url }),
    });
