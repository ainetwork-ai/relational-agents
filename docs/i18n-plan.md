...
- [ ] D6-3 `en.ts` 채우기 — 영어판 캡처(Q1) 대기. 캡처 오면 `scripts/i18n-keys.mjs --json` 의 `missing` 이 작업 목록.

### 결정을 남긴 것 (D5 관련, 지금은 노션 방식대로 "만들 때의 언어로 저장")
- 새 속성의 기본 이름이 유형 라벨(`텍스트`, `숫자`…)로 저장된다(전에는 `Text`).
- 새 페이지 기본 제목: `database-block.tsx` 는 `Untitled` 그대로, `chat-view.tsx` 의 "페이지로 만들기"는 `제목 없음`. 한쪽으로 맞추려면 결정 필요.
- 멘션 칩의 `제목 없음` 은 블록 HTML 에 박힌다.
