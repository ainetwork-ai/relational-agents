...
`POST /api/saveTransactions` — 노션의 `saveTransactionsFanout` 처럼 **페이지와 무관한 엔드포인트 하나**.
본문 `{ requestId, transactions: Transaction[] }`(트랜잭션마다 `pageId`), 응답은 **`{}`(200)** — 노션 실측과
같다. 권한은 페이지별로 확인한다(세션 또는 공유 토큰의 edit 권한, `restricted` 페이지의 `pageMembers`).
영영 적용될 수 없는 트랜잭션(권한 없음, 형식 오류)이 있으면 **4xx** 에 `{ errorId, name, message, rejectedIds }`
로 그 id 들을 이름 붙인다(노션 API 오류 형태를 따랐지만 이 부분은 실측하지 못한 우리 선택이다). 클라이언트는
이름 붙은 것만 큐에서 빼고 배지로 알리며, 나머지는 재시도한다(이미 적용된 것은 멱등이라 무해). 401 은
세션이 돌아올 수 있으니 버리지 않고 재시도한다.
...
| 단계 | 내용 | 끝났다는 기준(§7) |
|---|---|---|
| **1 (2026-09-08 완료)** | 블록 단위 연산(`set`/`update`/`alive`), IndexedDB 큐(§5 전부), 멱등 서버(§4.2·4.5, 텍스트는 `update {content}` LWW), 옛 초안 회수 | 요청 크기·지연·배치·로컬 보관·재시도·멱등·탭 닫기·배지 |
| **2 (2026-09-08 완료)** | 전역 엔드포인트 하나 + `{}` 응답(§4.1), 서버 안 트랜잭션 변환(§4.7: `PUT/POST /blocks`, 히스토리 복원), `alive` 소프트 삭제(§1.1, 읽기 14 곳 필터), SSE 로 트랜잭션 팬아웃(§4.4; 캐럿이 있는 블록의 내용 변경만 다음 전체 동기화로 미룸) | 원격 수신이 전체 GET 없이 반영, 응답 본문 `{}`, 삭제가 GET 에서 사라지되 행은 남음 |
