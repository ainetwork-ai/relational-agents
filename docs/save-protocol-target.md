...
1. `mutate` 가 트랜잭션을 만든다 → **먼저** `Transaction` 에 넣는다 → 화면 상태에 적용한다 → 큐에 알린다.
...
- 탭이 열리면 `Session` 에 자기 레코드를 만들고 3 초마다 `updatedAt` 을 갱신한다.
- 시작 1 초 뒤, 그리고 3 초마다, `updatedAt` 이 **8 초** 넘게 멈춘 세션을 찾아 그 세션의 트랜잭션을
  자기 세션으로 옮겨(`ownerSessionId` 갱신) 보낸다. 실측에서 새 탭이 고아를 회수해 보내기까지 약 8 초였고,
  이 값들로 회수는 탭이 열린 뒤 8~11 초 안에 일어난다.
...
| 지연 | 조용할 때 첫 입력 → 요청 ≤ 50ms; 응답 후 다음 요청 450~600ms; 동시 진행 1 |
