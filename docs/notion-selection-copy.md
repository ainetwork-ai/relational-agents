...
1. **전환 조건**: 텍스트에서 시작한 드래그는 텍스트 블록을 지나는 동안 네이티브 선택 유지. 비텍스트 블록에
   닿으면 그 시점부터 블록 선택. 여백 시작·비텍스트 클릭은 지금처럼 블록 선택.
2. **표시**: halo `rgba(35,131,226,0.14)`, radius 4px, ring 없음, inset 1–2px. `::selection` `rgba(35,131,226,0.28)`.
3. **복사**: 블록 선택 ⌘C → `text/plain`(마크다운, 자식 포함) + `text/html`(시맨틱) + 내부 블록 트리
   (`text/_ainmem-blocks-v1`, 붙여넣기가 먼저 읽음). 블록을 넘는 텍스트 선택 ⌘C → 같은 마크다운/HTML 규칙으로
   부분 텍스트 직렬화. ⌘X = 복사 + 삭제.
4. 측정 안 한 것: 드래그 시작 임계 픽셀, Escape 의 정확한 의미(G 가 불확정), 러버밴드 시각 요소(없는 것으로 관측).
