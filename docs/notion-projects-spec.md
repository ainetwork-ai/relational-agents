...
- **Status 셀의 드롭다운** — 원본과 많이 다르다고 확인받았다(2026-08-06). 아직 어디가 어떻게
  다른지 재지 않았고, 재고 나서 손댄다. 잴 것: 열리는 위치·너비·최대 높이, 그룹
  (`Not started`/`In progress`/`Done`) 머리글과 순서, 각 옵션 행의 구성(색 점·이름·오른쪽 아이콘),
  검색창 유무, 하단 액션(`+ 옵션 추가`/편집), 값이 있는 셀과 빈 셀의 차이
