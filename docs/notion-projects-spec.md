...
우리 구현: 같은 배치로 넣었고 `클립보드에 복사`는 동작한다. `댓글`은 셀 댓글이 없어서 비활성
(툴팁으로 이유 표시). 텍스트 계열(`text`·`url`·`email`·`phone`·`formula`·`rollup`)은 원본에서
값 있는 셀을 못 찾아 확인 못 했고, 복사 가능한 스칼라로 보아 같은 조합을 적용해 뒀다.
