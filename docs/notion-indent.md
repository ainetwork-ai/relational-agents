...
| 리스트 없이 들여쓴 문단들 | 한 블록의 줄바꿈으로 합쳐진다 | **다름** — 우리는 문단을 따로 만든다(§8) |
...
한 단 **4칸**, 할 일은 `- [ ]  `(상자 뒤 **두 칸**), 토글은 **글머리**로 나간다. 우리도 같은
형식으로 쓴다(`blocksToMarkdown`, `md-mirror`), 그래서 **왕복**이 성립한다 — 노션이 쓴 것을
우리가 읽어도, 우리가 쓴 것을 우리가 읽어도 같은 트리다(`e2e/markdown-nesting.check.mjs`).
...
- **리스트 없이 들여쓴 문단**: 노션은 `para-A` / `    para-B` 를 한 블록의 줄바꿈으로 합친다
  (CommonMark 의 lazy continuation). 우리는 문단 두 개로 둔다 — OKF 파일과 메모리 가져오기가
  줄 단위로 블록을 만드는 전제에 기대고 있어, 그 전제까지 바꾸는 건 이번 범위 밖이다.
- **콜아웃의 첫 자식에서 Backspace**: 노션은 콜아웃 앞 블록과 합치고 콜아웃을 없앤다.
  우리 콜아웃은 자기 텍스트를 들고 있는 모델이라(Enter 를 누르기 전) 경로가 다르다.
- md-mirror 는 워크스페이스를 통째로 다시 쓰면서 `<ws>.tmp-*` → rename 을 쓰는데, 실행이
  겹치면 rename 이 실패해 `.tmp-*` 디렉터리가 남는다(지금도 여러 개 남아 있다). 중첩과는
  무관한 별개 버그.
