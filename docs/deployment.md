...
### 3.8 로그인은 지갑에서 구글로 — 먼저 걷어냈다

2026-08-03. 있던 경로는 셋이었다: 데모 로그인(누구나 `DemoUser`), MetaMask 서명,
AIN 개인키 붙여넣기. 구글 로그인으로 간다는 결정에 따라 **먼저 전부 제거했다** —
지운 것은 라우트 5개(`demo-login`, `key-login`, `metamask-verify`, `challenge`,
`verify`), 검증 라이브러리 2개(`ain-verify`, `eth-verify`), 로그인 폼의 버튼·입력,
세션의 `ainAddress`·`challenge` 필드, `applyLoginDisplayName`, 그리고 e2e의
`eth-helpers.ts`(이미 아무도 안 쓰던 죽은 코드).

**지금 사이트에는 들어갈 방법이 없다.** 로그인 화면은 "재연결 중" 문구만 띄운다.
비활성 버튼을 두지 않은 이유는, 눌리지 않는 로그인 버튼이 빈 화면보다 나쁘기
때문이다 — 사람들이 원인을 찾아 헤맨다.

구글 로그인을 붙일 때 걸릴 것 두 가지를 미리 적어 둔다:

- **`users.ain_address`가 `NOT NULL UNIQUE`다.** 사람의 정체성이 지갑 주소였다.
  구글은 이메일이 정체성이므로 `email` 컬럼이 필요하고, `ain_address`는 nullable로
  풀리거나 에이전트 전용이 되어야 한다(에이전트 행은 provision 때 주소를 만든다).
  **prod DB가 아직 비어 있어 지금이라면 마이그레이션이 아니라 그냥 스키마 변경이다.**
- **`page_invites`는 이미 이메일로 초대한다.** 초대받은 사람이 같은 이메일로
  로그인하면 매칭되는 구조인데, `users`에 이메일이 없어 지금은 성립하지 않는다.
  구글 로그인이 이 구멍을 자연스럽게 메운다.

지갑이 사라지면서 **EIP-712 동의(consent)·dissolve와 온체인 릴레이도 실질적으로
동작하지 않는다** — 사람에게 서명할 주소가 없다. 코드(`consent/route.ts`,
`relation-registry.ts`, `wallet-sign-demo`)는 아직 남아 있다. 정리 여부는 열린
항목이다(§6-8).
...
3. **로그인이 없다.** 지갑 계열 경로를 걷어낸 뒤(§3.8) 구글 로그인이 아직 안 붙어서
   **아무도 들어갈 수 없다.** 사이트는 공개돼 있고 로그인 화면만 뜬다.
...
8. **지갑이 빠진 자리.** 구글 로그인이 붙어도 EIP-712 동의·dissolve·온체인 릴레이는
   서명할 주소가 없어 동작하지 않는다(§3.8). `consent/route.ts`,
   `relation-registry.ts`, `/wallet-sign-demo`, `users.encrypted_private_key`를
   같이 걷어낼지, 지갑 연결을 로그인과 분리해 남겨둘지 정해야 한다.
9. **e2e 로그인.** 남은 스펙 3개(CHAT-fix ×2, EMOJI ×1)는 세션이 필요해서 지금
   `helpers.ts`의 `demoLogin()`이 `test.skip()`으로 건너뛴다. 구글 로그인이 붙으면
   그 함수 본문만 바꾸면 스펙은 그대로 돈다.
