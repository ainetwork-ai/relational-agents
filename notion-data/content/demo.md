...
# Demo — "human to human, with an Agent that Remembers"
...
## 0:00–0:20 · Opening — an invitation, not a page
...
## 0:20–0:50 · Mutual consent → the relationship is born
...
## 0:45–1:30 · It manages (real data on screen)
...
## 1:30–2:05 · It remembers (quotes the records)
...
## 2:05–2:45 · Finale: a loyal agent (the fun bit)
...
1. **Cold open → Invite (0:00)** — fade in from black 0.6s onto a near-empty Chats sidebar; the cursor moves straight to the **+**. No table yet — the first gesture is reaching out.
2. **Invite → Consent (0:20)** — on Isla's *accept* the New-DM modal doesn't just close; its avatar **match-cuts into the freshly-created sidebar row** **Chanho ❤️ Isla Montgomery** (0.4s). The page is born from the invitation.
...
4. **Consent → Manage (0:45)** — **match cut** on the agent avatar: chat bubble avatar aligns to the same position as the briefing card avatar. 0.4s crossfade.
...
6. **Manage → Remember (1:30)** — **whip pan** from Sophie's page to the agent chat (0.25s, motion blur): "turning to ask."
7. **Inside Remember** — as the agent quotes Isla, **push-zoom 110%** onto the quoted chat lines in her record for 1s, then settle.
8. **Remember → Egg tart (2:05)** — no transition. **Hard cut + kill the BGM.** Dry silence sets up the joke.
...
10. **Egg tart → Closing (2:45)** — luma fade to white 0.8s into the graph zoom-out; warm BGM returns.
11. **Title card (2:57)** — graph nodes collapse into the logo dot (0.5s), title in, hold 2s, fade out.
